...
$Env:WINDOWS_SIGN_KEYSTORE_ENCODED = "keystore.p12.base64"
$Env:WINDOWS_SIGN_KEYSTORE = "keystore.p12"

if($Env:WINDOWS_SIGN_KEYSTORE_BASE64)
{
    Set-Content -Path $Env:WINDOWS_SIGN_KEYSTORE_ENCODED -Value $Env:WINDOWS_SIGN_KEYSTORE_BASE64
    certutil -decode $Env:WINDOWS_SIGN_KEYSTORE_ENCODED $Env:WINDOWS_SIGN_KEYSTORE
    certutil -p $Env:WINDOWS_SIGN_PASSWORD -dump $Env:WINDOWS_SIGN_KEYSTORE | Tee-Object -FilePath "CertDump.txt"

    $expiryLine = Get-Content "CertDump.txt" | Where-Object { $_ -match "NotAfter:" }
    if($expiryLine)
    {
        $expiryDateStr = $expiryLine -replace ".*NotAfter:\s*", ""
        $expiryDate = Get-Date $expiryDateStr
        $currentDate = Get-Date

        if($currentDate -gt $expiryDate)
        {
            Write-Host "Certificate expired, signing disabled..."
        }
        else
        {
            certutil -p $Env:WINDOWS_SIGN_PASSWORD -importPFX $Env:WINDOWS_SIGN_KEYSTORE
            $Env:SIGNING_ENABLED = "true"
            Write-Host "Signing enabled..."
        }
    }

    Remove-Item $Env:WINDOWS_SIGN_KEYSTORE_ENCODED, $Env:WINDOWS_SIGN_KEYSTORE
}
...
$SIGNTOOL_ARGS = "sign /fd SHA256 /tr ${Env:WINDOWS_SIGN_TSA} /td SHA256 /sm /n `"${Env:WINDOWS_SIGN_SUBJECTNAME}`""
if(${Env:SIGNING_ENABLED})
{
    Write-Host "------ signing executables"
    foreach($exe in "${PRODUCT_NAME}", "CrashReporter", "MessageBox", "Updater")
    {
        & signtool $SIGNTOOL_ARGS "$INSTALLER_DIR\$exe.exe"
    }
}
...
if(${Env:SIGNING_ENABLED})
{
    Write-Host "------ signing uninstaller"
    & signtool $SIGNTOOL_ARGS "$INSTALLER_DIR\Uninstall.exe"
}
...
Write-Host "------ making final installer"
...
if(${Env:SIGNING_ENABLED})
{
    & signtool $SIGNTOOL_ARGS $INSTALLER_EXE
}

Copy-Item $INSTALLER_EXE $BUILD_DIR -Verbose
