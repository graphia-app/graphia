...
ApplicationWindow
{
    ...
    function processOnePendingArgument()
    {
        ...
        let url = NativeUtils.urlForUserInput(argument);
    ...
    Component.onCompleted:
    {
        ...
        if(!misc.hasSeenTutorial)
        {
            ...
            if(NativeUtils.fileExists(exampleFile))
            {
    ...
    function openUrl(url, inNewTab)
    {
        ...
        if(NativeUtils.urlIsFile(url) && !NativeUtils.fileUrlExists(url))
        {
            errorOpeningFileMessageDialog.title = qsTr("File Not Found");
            errorOpeningFileMessageDialog.text =
                Utils.format(qsTr("{0} does not exist."), NativeUtils.baseFileNameForUrl(url));
            errorOpeningFileMessageDialog.open();
            return;
        }
        ...
        if((types.length === 0 || !application.canOpenAnyOf(types)) && NativeUtils.urlIsDownloadable(url))
        {
        ...
        if(types.length === 0)
        {
            ...
            {
                errorOpeningFileMessageDialog.title = NativeUtils.urlIsFile(url) ?
        ...
        if(types.length > 1)
        {
            let extension = NativeUtils.extensionForUrl(url);
    ...
    Action
    {
        ...
        onTriggered:
        {
            ...
            if(NativeUtils.fileUrlExists(infiniteParseUrl))
        ...
        PlatformMenu
        {
            ...
            PlatformMenuItem
            {
                ...
                onTriggered:
                {
                    ...
                    if(NativeUtils.fileUrlExists(exampleFileUrl))
                    {
    ...
    DropArea
    {
        ...
        onDropped: function(drop)
        {
            ...
            if(drop.hasText && drop.text.length > 0)
            {
                ...
                text = NativeUtils.urlIsValid(text) ? Qt.resolvedUrl(text) : text;
                arguments.push(text);
            }
                ...
                onLoadComplete: function(url, success)
                {
                    if(success)
                    {
                        ...
                        if(application.isResourceFileUrl(url) &&
                            NativeUtils.baseFileNameForUrlNoExtension(url) === "Tutorial")
                        {
